<template>
    <nav class="px-10 py-2">
        <Nuxt-Link to="/likedcountries" class="flex items-center gap-1 border border-red-500 rounded-full w-14 aspect-square p-1" >
            <svg class="w-10 h-10" version="1.0" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
            ...
            <span>{{ store.likedCountriesCodes.length }}</span>
        </Nuxt-Link>
    </nav>
    <div class="container">
        <ul class="flex flex-wrap list-none p-0 w-full">
            <CountryCard v-if="countries.length" :countries="countries" />
        </ul>
    </div>
</template>
